...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  //   3) generate user the email
  try {
    //Manually->
    // const resetURL = `${req.protocol}://${req.get(
    //   "host"
    // )}/api/v1/users/resetPassword/${resetToken}`;
    // const message = `Forgot your password ? Submit a PATCH request with your new Password and passwordConfirm to ${resetURL}.IF you didnt forget your password ,please ignore this`;
    // await sendEmail({
    //   email: user.email,
    //   subject: "Your password is valid only for 10mins",
    //   message,
    // });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  //1) Get User based on the token and password
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  //3) Update changedPasswordAt property for the user and
